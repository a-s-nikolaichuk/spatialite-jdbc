...
JDBC_API:=lib/jdbc-api-1.4.jar
PRECOMPILED_CLASSES:=src/main/resources/
...
$(ICONV_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://ftp.gnu.org/pub/gnu/libiconv/libiconv-$(ICONV_VERSION).tar.gz
...
$(GEOS_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://download.osgeo.org/geos/geos-$(GEOS_VERSION).tar.bz2
...
$(PROJ_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://download.osgeo.org/proj/proj-$(PROJ_VERSION).tar.gz
...
$(LIBXML2_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://xmlsoft.org/sources/libxml2-$(LIBXML2_VERSION).tar.gz
...
$(ZLIB_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://fossies.org/linux/misc/zlib-$(ZLIB_VERSION).tar.gz
...
$(LZMA_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://tukaani.org/lzma/lzma-$(LZMA_VERSION).tar.gz
...
$(SQLITE_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://www.sqlite.org/2016/$(SQLITE_AMAL_PREFIX).zip

$(SQLITE_UNPACKED): $(SQLITE_ARCHIVE)
	unzip -qo $< -d $(TARGET)
	touch $@

SPATIALITE_ARCHIVE:=$(TARGET)/libspatialite-$(SPATIALITE_VERSION)$(SPATIALITE_REVISION).zip
...
$(SPATIALITE_ARCHIVE):
	@mkdir -p $(@D)
	curl -L -o $@ http://www.gaia-gis.it/gaia-sins/libspatialite-sources/libspatialite-$(SPATIALITE_VERSION)$(SPATIALITE_REVISION).zip
...
$(OUT_DIR)/org/spatialite/%.class: src/main/java/org/spatialite/%.java
	@mkdir -p $(@D)
	$(JAVAC) -source 1.5 -target 1.5 -sourcepath $(SRC) -classpath $(PRECOMPILED_CLASSES) -Xbootclasspath/p:$(JDBC_API) -d $(OUT_DIR) $<

jni-header: $(OUT_DIR)/NativeDB.h $(OUT_DIR)/NestedDB.h

$(OUT_DIR)/NativeDB.h: $(OUT_DIR)/org/spatialite/NativeDB.class
	$(JAVAH) -classpath $(OUT_DIR) -jni -o $@ org.spatialite.NativeDB

$(OUT_DIR)/NestedDB.h: $(OUT_DIR)/org/spatialite/NestedDB.class
	$(JAVAH) -classpath $(OUT_DIR) -jni -o $@ org.spatialite.NestedDB
...
$(OUT_DIR)/$(LIBNAME): $(OUT_DIR)/sqlite3.o $(SRC)/org/spatialite/NativeDB.c $(SRC)/org/spatialite/NestedDB.c $(OUT_DIR)/NativeDB.h $(OUT_DIR)/NestedDB.h
	@mkdir -p $(@D)
	$(CC) $(CFLAGS) -c -o $(OUT_DIR)/NativeDB.o $(SRC)/org/spatialite/NativeDB.c $(SPATIALITE_FLAGS)
	$(CC) $(CFLAGS) -c -o $(OUT_DIR)/NestedDB.o $(SRC)/org/spatialite/NestedDB.c $(SPATIALITE_FLAGS)
